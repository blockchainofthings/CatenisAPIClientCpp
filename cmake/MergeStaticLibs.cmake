...
# Merge_static_libs(output_library mylib lib1 lib2 ... libn) merges a number of static
...
function(merge_static_libs output_library)
  ...
  foreach(lib ${libs})
    # For some reason openssl libs type is unknown so comment this part for now
    #message(status "lib: " ${lib})
    #get_target_property(libtype ${lib} TYPE)
    #message(status "lib: " ${libtype})
    #if(NOT libtype STREQUAL "STATIC_LIBRARY")
    #  message(FATAL_ERROR "Merge_static_libs can only process static libraries")
    #endif()
    if(multiconfig)
      foreach(CONFIG_TYPE ${CMAKE_CONFIGURATION_TYPES})
        # mylib will not have that property and throw error
        if(NOT (${lib} STREQUAL ${mylib}))
          get_target_property("libfile_${CONFIG_TYPE}" ${lib} "LOCATION_${CONFIG_TYPE}")
	  #message(status "lib_config: " ${libfile_${CONFIG_TYPE}})
          list(APPEND libfiles_${CONFIG_TYPE} ${libfile_${CONFIG_TYPE}})
	else()
	  list(APPEND libfiles_${CONFIG_TYPE} ${tempCatenis_loc}) 
        endif()
      endforeach()
    else()
      #message(status "lib: " ${lib})
      if(${lib} STREQUAL ${mylib})
        set(libfile ${tempCatenis_loc})
      else()
  ...
  foreach(CONFIG_TYPE ${CMAKE_CONFIGURATION_TYPES})
    ...
    foreach(lib ${libfiles_${CONFIG_TYPE}})
      set(flags "${flags} ${lib}")
      #message(status ": ${CONFIG_TYPE} ${lib}")
    endforeach()
  ...
  #set(outfile $<TARGET_FILE:${output_target}>)
  set(outfile ${catenic_loc})
